...
class TweetSetSuite extends FunSuite {
  ...
  test("mostRetweeted: throw NoSuchElementException when EmptySet") {
    new TestSets {
  ...
  test("descending: set5") {
    new TestSets {
      ...
      assert(!trends.isEmpty, "is empty")
  ...
  test("filter: tweet with 321 retweets") {
    assert(size(TweetReader.allTweets.filter(t => t.retweets == 321)) === 1)
  }
}
